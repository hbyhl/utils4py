...
# 偷巧方法，未调整astime的输出格式为"%Y-%m-%d %H:%M:%S.03d" 二不是"%Y-%m-%d %H:%M:%S,03d"
...
def init_log_from_config(cfg_file, formatter='flume', delay=False):
    ...
    log_dir = os.path.abspath(parser.get(BASE, LOG_DIR))
